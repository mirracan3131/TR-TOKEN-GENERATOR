...
const captchakey = '4c672d35-0701-42b2-88c3-78380b0db560'
const PROXY_ADDR = ''
...
if (repeat < 2) {
  ...
  async function fill_discord(DiscordPage, username, password, email){
    ...
    await click_date(DiscordPage, "Year", 11,11 );
  ...
  async function create_accinfos(browser, d) {
    // Options
    const isimler = ["Alimert","Alimehmet","Alisemih","Alisvs","Ali58","Alikaradağ","Semiha","Serhet","efesvs","efetr","erkan","Sümeyyekılı","Selmaista","Selinsalı","Serkan"];
    const username = isimler[Math.floor(Math.random() * isimler.length)];
    const password = "Malefe!";
